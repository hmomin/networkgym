...
class LSPI_BC:
    ...
    def state_featurizer_forward(self, states: torch.Tensor) -> torch.Tensor:
        output = states
    ...
    def compute_kl_divergence_loss(self, states: torch.Tensor, actions: torch.Tensor):
        actor_logits = self.actor.forward(states.float())
        actor_probs = F.softmax(actor_logits, dim=1)
        log_probs = torch.log(actor_probs)
        behavior_one_hots = F.one_hot(
            actions.flatten().to(torch.int64), num_classes=self.num_actions
        )
        neg_log_p = torch.sum(behavior_one_hots * -log_probs, dim=1)
        policy_loss = torch.mean(neg_log_p)
        return policy_loss
    ...
    def compute_LSPI_weights(self) -> None:
        ...
        while rank_A_tilde < self.k:
            ...
            actions = F.one_hot(
                discrete_actions.flatten().to(torch.int64), num_classes=self.num_actions
            )
